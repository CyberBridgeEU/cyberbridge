...
import { cyberbridge_back_end_rest_api, osv_rest_api_wrapper } from "../constants/urls.ts";
...
const useOsvStore = create<UseOsvStore>((set, get) => ({
    // variables
    scanResults: null,
    loading: false,
    error: null,
    ...
    checkHealth: async () => {
        ...
        try {
            const response = await fetch(`${osv_rest_api_wrapper}/`, {
            ...
            console.log('OSV API health check response:', data);
            set({
                loading: false,
            ...
            console.error('Error checking OSV API health:', error);
            set({
    ...
    scanZipFile: async (file: File, useLlm: boolean = true) => {
        ...
        try {
            ...
            const response = await fetch(`${cyberbridge_back_end_rest_api}/scanners/osv/scan?use_llm=${useLlm}`, {
                method: 'POST',
                headers: {...useAuthStore.getState().getAuthHeader()},
                body: formData,
    ...
    scanGithubRepo: async (githubUrl: string, useLlm: boolean = true, githubToken?: string) => {
        ...
        try {
            ...
            let url = `${cyberbridge_back_end_rest_api}/scanners/osv/scan-github?github_url=${encodeURIComponent(githubUrl)}&use_llm=${useLlm}`;
