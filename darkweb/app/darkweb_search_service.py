...
from app.categorized_keyword_search import main_categorized_search
import pathlib
import socket
import time
import platform
...
def search_keyword(keyword, engines=None, exclude=None, mp_units=2, proxy="localhost:9050", limit=3, continuous_write=False, use_categorized=False):
    """
    Function to perform a dark web search with timing metrics.
    
    :param use_categorized: If True, uses the new categorized search with PDF report generation
    """
    timing_results = {}
    ...
    # Run search function - choose between categorized or original
    start_time = time.time()
    if use_categorized:
        # Pass engine status information to categorized search
        engine_status = {"up_engines": up_engines, "down_engines": down}
        output_json = main_categorized_search(args.search, args.mp_units, new_search.results, engine_status)
    else:
    ...
    if use_categorized and isinstance(output_json, dict):
        output_json["timing_metrics"] = timing_results
